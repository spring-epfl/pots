"""
MILP solver for the network interdiction problem against traffic routing apps.
"""
...
class EdgeData:
    ...
    # Delay that an intersection produces in seconds.
...
def get_time_delta(edge_data, target_speed_lim=MIN_SPEED_LIM):
    """What is the time difference corresponds to the change in speed limit."""
...
def solve_milp(
    ...
    Args:
        ...
        edges (dict): Traphic graph arcs. Mapping of node tuples that represent node
                identifiers (a, b) to :py:class:`EdgeData` objects.
        min_time: Minimum time to pass through town in hours
        delta: Speed limit delta for any interdiction, or a function ("scheduler") that takes an
            :py:class:`EdgeData` object and returns the delta.
