...
def solve_milp(
    ...
    for edge, edge_data in edges.items():
        ...
        if updated_speed_lim >= MIN_SPEED_LIM:
            solver.Add(q - p - interdiction_var * time_delta <= edge_data.get_time())
        else:
...
def solve_lp(nodes, edges, min_time, source="S", target="T", verbose=False):
    # Initialize solver.
    solver = pywraplp.Solver("waze", pywraplp.Solver.GLOP_LINEAR_PROGRAMMING)
    ...
    interdiction_vars_by_arc = {}
    for i, (edge, edge_data) in enumerate(edges.items()):
        upper_lim = get_time_delta(edge_data)
    ...
    for edge, edge_data in edges.items():
        ...
        p = path_vars_by_node[edge[0]]
        q = path_vars_by_node[edge[1]]
        solver.Add(q - p - interdiction_var <= edge_data.get_time())
